...
// dominantResourceShare 返回一个 0 到 1,000,000 的值，表示所有资源中超出 nominal quota 的使用量与 cohort 可借用资源的最大比值，并除以权重。如果为 0，表示 ClusterQueue 的使用量低于 nominal quota。函数还返回导致该值的资源名。当 FairSharing 权重为 0 且 ClusterQueue 或 Cohort 处于借用状态时，返回 math.MaxInt。
