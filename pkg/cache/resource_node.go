...
type ResourceNode struct { // 所有 flavor 的资源情况
	Quotas       map[over_resources.FlavorResource]ResourceQuota // flavorName+resourceName: Quota
	SubtreeQuota over_resources.FlavorResourceQuantities         // cq.SubtreeQuota=cq.Nominal ;cohort.SubtreeQuota = sum([flavor.lendingLimit])
	Usage        over_resources.FlavorResourceQuantities         // 当前cq 使用的资源，会超出 上限-借出     ； cohort 用的(借用总和)
}

func NewResourceNode() ResourceNode {
	return ResourceNode{
		Quotas:       make(map[over_resources.FlavorResource]ResourceQuota),
		SubtreeQuota: make(over_resources.FlavorResourceQuantities),
...
func QuantitiesFitInQuota(node flatResourceNode, requests over_resources.FlavorResourceQuantities) (bool, over_resources.FlavorResourceQuantities) {
	fits := true
	remainingRequests := make(over_resources.FlavorResourceQuantities, len(requests))
...
func IsWithinNominalInResources(node flatResourceNode, frs sets.Set[over_resources.FlavorResource]) bool {
	for fr := range frs {
...
func updateCohortResourceNode(cohort *cohort) { // ✅
	cohort.resourceNode.SubtreeQuota = make(over_resources.FlavorResourceQuantities, len(cohort.resourceNode.SubtreeQuota))
	cohort.resourceNode.Usage = make(over_resources.FlavorResourceQuantities, len(cohort.resourceNode.Usage))
...
func available(node hierarchicalResourceNode, fr over_resources.FlavorResource) int64 {
	// 获取当前节点的资源信息
...
func updateClusterQueueResourceNode(cq *clusterQueue) {
	...
	cq.resourceNode.SubtreeQuota = make(over_resources.FlavorResourceQuantities, len(cq.resourceNode.Quotas))
...
func (r ResourceNode) guaranteedQuota(fr over_resources.FlavorResource) int64 {
...
func addUsage(node hierarchicalResourceNode, fr over_resources.FlavorResource, val int64) {
	r := node.getResourceNode()
...
func LocalAvailable(node flatResourceNode, fr over_resources.FlavorResource) int64 {
...
func removeUsage(node hierarchicalResourceNode, fr over_resources.FlavorResource, val int64) {
	r := node.getResourceNode()
...
func potentialAvailable(node hierarchicalResourceNode, fr over_resources.FlavorResource) int64 {
	r := node.getResourceNode()
	if !node.HasParent() {
