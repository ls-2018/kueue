...
func ValidateFeatureGates(featureGateCLI string, featureGateMap map[string]bool) error {
	...
	TASProfilesEnabled := []bool{
		features.Enabled(features.TASProfileMixed),             // 启用为 TAS 使用混合算法（BestFit 或 LeastFreeCapacity），根据 TAS 需求级别切换算法。
		features.Enabled(features.TASProfileLeastFreeCapacity), // 启用为 TAS 使用 LeastFreeCapacity 算法。
	}
