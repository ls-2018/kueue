...
func NewWorkloadReconciler(client client.Client, queues *over_queue.Manager, cache *cache.Cache, recorder record.EventRecorder, opts ...Option) *WorkloadReconciler {
...
func (r *WorkloadReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if finishedCond != nil && finishedCond.Status == metav1.ConditionTrue {
		if !over_features.Enabled(over_features.ObjectRetentionPolicies) || r.workloadRetention == nil || r.workloadRetention.afterFinished == nil {
		...
		if updated {
			...
			if evicted && wl.Status.Admission != nil {
				...
				if reportWorkloadEvictedOnce {
					over_metrics.ReportEvictedWorkloadsOnce(wl.Status.Admission.ClusterQueue, kueue.WorkloadDeactivated, "")
				}
			}
			return ctrl.Result{}, nil
		}
	...
	if !workload.IsAdmitted(&wl) && workload.SyncAdmittedCondition(&wl, r.clock.Now()) {
		...
		if workload.IsAdmitted(&wl) {
			...
			over_metrics.AdmittedWorkload(cqName, queuedWaitTime)
			over_metrics.AdmissionChecksWaitTime(cqName, quotaReservedWaitTime)
			if over_features.Enabled(over_features.LocalQueueMetrics) {
				over_metrics.LocalQueueAdmittedWorkload(over_metrics.LQRefFromWorkload(&wl), queuedWaitTime)
				over_metrics.LocalQueueAdmissionChecksWaitTime(over_metrics.LQRefFromWorkload(&wl), quotaReservedWaitTime)
			}
		}
		return ctrl.Result{}, nil
	}
...
func (r *WorkloadReconciler) reconcileCheckBasedEviction(ctx context.Context, wl *kueue.Workload) (bool, error) {
	...
	if reportWorkloadEvictedOnce {
		over_metrics.ReportEvictedWorkloadsOnce(wl.Status.Admission.ClusterQueue, kueue.WorkloadEvictedByAdmissionCheck, "")
	}
	return true, nil
}
...
func (r *WorkloadReconciler) reconcileSyncAdmissionChecks(ctx context.Context, wl *kueue.Workload, cq *kueue.ClusterQueue) (bool, error) {
	log := ctrl.LoggerFrom(ctx)
	admissionChecks := workload.AdmissionChecksForWorkload(log, wl, over_admissioncheck.NewAdmissionChecks(cq))
	...
	if workload.IsAdmitted(wl) {
		...
		if err == nil {
			...
			if slices.Contains(r.queues.GetClusterQueueNames(), cqName) {
				over_metrics.ReportEvictedWorkloads(cqName, kueue.WorkloadEvictedByLocalQueueStopped)
				if over_features.Enabled(over_features.LocalQueueMetrics) {
					over_metrics.ReportLocalQueueEvictedWorkloads(over_metrics.LQRefFromWorkload(wl), kueue.WorkloadEvictedByLocalQueueStopped)
				}
			}
		}
		return true, client.IgnoreNotFound(err)
...
func (r *WorkloadReconciler) reconcileOnClusterQueueActiveState(ctx context.Context, wl *kueue.Workload, cqName kueue.ClusterQueueReference) (bool, error) {
	...
	if workload.IsAdmitted(wl) {
		...
		if err == nil {
			...
			if reportWorkloadEvictedOnce {
				over_metrics.ReportEvictedWorkloadsOnce(wl.Status.Admission.ClusterQueue, kueue.WorkloadEvictedByClusterQueueStopped, "")
			}
		}
		return true, client.IgnoreNotFound(err)
	}
...
func (r *WorkloadReconciler) reconcileNotReadyTimeout(ctx context.Context, req ctrl.Request, wl *kueue.Workload) (time.Duration, error) {
	...
	if err == nil {
		...
		if reportWorkloadEvictedOnce {
			over_metrics.ReportEvictedWorkloadsOnce(cqName, kueue.WorkloadEvictedByPodsReadyTimeout, underlyingCause)
		}
	}
	return 0, client.IgnoreNotFound(err)
}
