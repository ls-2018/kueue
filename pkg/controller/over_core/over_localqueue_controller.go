...
func (r *LocalQueueReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if r.admissionFSConfig != nil && over_features.Enabled(over_features.AdmissionFairSharing) {
...
func (r *LocalQueueReconciler) Delete(e event.TypedDeleteEvent[*kueue.LocalQueue]) bool {
	if over_features.Enabled(over_features.LocalQueueMetrics) {
		over_metrics.ClearLocalQueueResourceMetrics(localQueueReferenceFromLocalQueue(e.Object))
	}
...
func (r *LocalQueueReconciler) reconcileConsumedUsage(ctx context.Context, lq *kueue.LocalQueue, cqName kueue.ClusterQueueReference) error {
	...
	scaledNewUsage := over_resource.MulByFloat(newUsage, alpha)
	scaledOldUsage := over_resource.MulByFloat(oldUsage, 1-alpha)
	sum := over_resource.MergeResourceListKeepSum(scaledOldUsage, scaledNewUsage)
	// update status
...
func localQueueReferenceFromLocalQueue(lq *kueue.LocalQueue) over_metrics.LocalQueueReference {
	return over_metrics.LocalQueueReference{
...
func recordLocalQueueUsageMetrics(queue *kueue.LocalQueue) {
	for _, flavor := range queue.Status.FlavorUsage {
		for _, r := range flavor.Resources {
			over_metrics.ReportLocalQueueResourceUsage(localQueueReferenceFromLocalQueue(queue), string(flavor.Name), string(r.Name), over_resource.QuantityToFloat(&r.Total))
		}
	}
	for _, flavor := range queue.Status.FlavorsReservation {
		for _, r := range flavor.Resources {
			over_metrics.ReportLocalQueueResourceReservations(localQueueReferenceFromLocalQueue(queue), string(flavor.Name), string(r.Name), over_resource.QuantityToFloat(&r.Total))
		}
	}
}

func updateLocalQueueResourceMetrics(queue *kueue.LocalQueue) {
	over_metrics.ClearLocalQueueResourceMetrics(localQueueReferenceFromLocalQueue(queue))
...
func (r *LocalQueueReconciler) UpdateStatusIfChanged(
	...
	if len(conditionStatus) != 0 && len(reason) != 0 && len(msg) != 0 {
		...
		if over_features.Enabled(over_features.LocalQueueMetrics) {
			over_metrics.ReportLocalQueueStatus(over_metrics.LocalQueueReference{
