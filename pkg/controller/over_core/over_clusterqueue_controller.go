...
func (r *ClusterQueueReconciler) Delete(e event.TypedDeleteEvent[*kueue.ClusterQueue]) bool {
	...
	over_metrics.ClearClusterQueueResourceMetrics(e.Object.Name)
...
func recordResourceMetrics(cq *kueue.ClusterQueue) {
	for rgi := range cq.Spec.ResourceGroups {
		...
		for fqi := range rg.Flavors {
			...
			for ri := range fq.Resources {
				r := &fq.Resources[ri]
				nominal := over_resource.QuantityToFloat(&r.NominalQuota)
				borrow := over_resource.QuantityToFloat(r.BorrowingLimit)
				lend := over_resource.QuantityToFloat(r.LendingLimit)
				over_metrics.ReportClusterQueueQuotas(cq.Spec.Cohort, cq.Name, string(fq.Name), string(r.Name), nominal, borrow, lend)
			}
		}
	}
	...
	for fri := range cq.Status.FlavorsReservation {
		...
		for ri := range fr.Resources {
			r := &fr.Resources[ri]
			over_metrics.ReportClusterQueueResourceReservations(cq.Spec.Cohort, cq.Name, string(fr.Name), string(r.Name), over_resource.QuantityToFloat(&r.Total))
		}
	}
	...
	for fui := range cq.Status.FlavorsUsage {
		...
		for ri := range fu.Resources {
			r := &fu.Resources[ri]
			over_metrics.ReportClusterQueueResourceUsage(cq.Spec.Cohort, cq.Name, string(fu.Name), string(r.Name), over_resource.QuantityToFloat(&r.Total))
		}
	}
}
...
func updateResourceMetrics(oldCq, newCq *kueue.ClusterQueue) {
	...
	if oldCq.Spec.Cohort != newCq.Spec.Cohort {
		over_metrics.ClearClusterQueueResourceMetrics(oldCq.Name)
	} else {
		// selective remove
...
func clearOldResourceQuotas(oldCq, newCq *kueue.ClusterQueue) {
	for rgi := range oldCq.Spec.ResourceGroups {
		...
		for fi := range oldRG.Flavors {
			...
			if newFlavor, found := newFlavors[flavor.Name]; !found || len(newFlavor.Resources) == 0 {
				over_metrics.ClearClusterQueueResourceQuotas(oldCq.Name, string(flavor.Name), "")
			} else {
				// check all resources
				...
				for ri := range flavor.Resources {
					...
					if _, found := newResources[rname]; !found {
						over_metrics.ClearClusterQueueResourceQuotas(oldCq.Name, string(flavor.Name), string(rname))
					}
				}
			}
		}
	...
	if len(oldCq.Status.FlavorsReservation) > 0 {
		...
		for fi := range oldCq.Status.FlavorsReservation {
			...
			if newFlavor, found := newFlavors[flavor.Name]; !found || len(newFlavor.Resources) == 0 {
				over_metrics.ClearClusterQueueResourceReservations(oldCq.Name, string(flavor.Name), "")
			} else {
				newResources := utilslices.ToRefMap(newFlavor.Resources, func(r *kueue.ResourceUsage) corev1.ResourceName { return r.Name })
				for ri := range flavor.Resources {
					rname := flavor.Resources[ri].Name
					if _, found := newResources[rname]; !found {
						over_metrics.ClearClusterQueueResourceReservations(oldCq.Name, string(flavor.Name), string(rname))
					}
				}
			}
		}
	...
	if len(oldCq.Status.FlavorsUsage) > 0 {
		...
		for fi := range oldCq.Status.FlavorsUsage {
			...
			if newFlavor, found := newFlavors[flavor.Name]; !found || len(newFlavor.Resources) == 0 {
				over_metrics.ClearClusterQueueResourceUsage(oldCq.Name, string(flavor.Name), "")
			} else {
				newResources := utilslices.ToRefMap(newFlavor.Resources, func(r *kueue.ResourceUsage) corev1.ResourceName { return r.Name })
				for ri := range flavor.Resources {
					rname := flavor.Resources[ri].Name
					if _, found := newResources[rname]; !found {
						over_metrics.ClearClusterQueueResourceUsage(oldCq.Name, string(flavor.Name), string(rname))
					}
				}
			}
		}
...
func (r *ClusterQueueReconciler) updateCqStatusIfChanged(
	...
	if r.fairSharingEnabled {
		if r.reportResourceMetrics {
			over_metrics.ReportClusterQueueWeightedShare(cq.Name, stats.WeightedShare)
		}
...
func (r *ClusterQueueReconciler) isVisibilityEnabled() bool {
	return over_features.Enabled(over_features.QueueVisibility) && r.queueVisibilityClusterQueuesMaxCount > 0
}
