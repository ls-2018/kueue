...
func cloneForCreate(orig *kueue.Workload, origin string) *kueue.Workload {
	...
	remoteWl.ObjectMeta = over_api.CloneObjectMetaForCreation(&orig.ObjectMeta)
...
func (w *wlReconciler) multikueueAC(ctx context.Context, local *kueue.Workload) (*kueue.AdmissionCheckState, error) {
	// 正确的检查
	relevantChecks, err := over_admissioncheck.FilterForController(ctx, w.localclient, local.Status.AdmissionChecks, kueue.MultiKueueControllerName) // ✅
	if err != nil {
		return nil, err
	}
...
func (w *wlReconciler) Reconcile(ctx context.Context, req reconcile.Request) (reconcile.Result, error) {
	...
	grp, err := w.readGroup(ctx, wl, mkAc.Name, adapter, owner.Name) // 每个集群的workload 的状态
	if err != nil {
		return reconcile.Result{}, err
	}
