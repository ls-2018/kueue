...
type Pod struct {
	...
	isFound               bool
	isGroup               bool // reconcileRequestForPod    pod 身上有 group label
...
var (
	...
	_ jobframework.JobWithFinalize                 = (*Pod)(nil)
	_ jobframework.ComposablePod                   = (*Pod)(nil)
...
func (p *Pod) RunWithPodSetsInfo(_ []over_podset.PodSetInfo) error {
...
func (p *Pod) RestorePodSetsInfo(_ []over_podset.PodSetInfo) bool {
...
func (p *Pod) Skip() bool {
	...
	if v, ok := p.pod.GetLabels()[over_constants.ManagedByKueueLabelKey]; p.isFound && (!ok || v != over_constants.ManagedByKueueLabelValue) {
		return true
	}
	return false
}
...
func prepare(pod *corev1.Pod, info over_podset.PodSetInfo) error {
	if err := over_podset.Merge(&pod.ObjectMeta, &pod.Spec, info); err != nil {
		return err
	}
...
func (p *Pod) Load(ctx context.Context, c client.Client, key *types.NamespacedName) (removeFinalizers bool, err error) {
	...
	if err := c.List(ctx, &p.list, client.MatchingFields{PodGroupNameCacheKey: key.Name}, client.InNamespace(key.Namespace)); err != nil {
		return false, err
	}
...
func (p *Pod) Stop(ctx context.Context, c client.Client, _ []over_podset.PodSetInfo, stopReason jobframework.StopReason, eventMsg string) ([]client.Object, error) {
	...
	for i := range podsInGroup {
		...
		if err := c.Status().Patch(ctx, pCopy, client.Apply, client.FieldOwner(over_constants.KueueName)); err != nil && !apierrors.IsNotFound(err) {
	...
	// 如果相关工作负载被删除，通用协调器将停止 Pod 组并完成工作负载。然而，它不会完成 Pod。由于 Pod 组的 Stop 方法会删除组中的所有 Pod，所以这里会完成 Pod。
...
func (p *Pod) Finalize(ctx context.Context, c client.Client) error {
	...
	if groupName == "" {
		...
		if err := c.List(ctx, &podsInGroup, client.MatchingFields{PodGroupNameCacheKey: groupName}, client.InNamespace(p.pod.Namespace)); err != nil {
			return err
		}
	}
...
// isPodRunnableOrSucceeded 返回该 Pod 是否最终能够运行、是否正在运行或是否已成功运行。
// 如果 Pod 被限制、没有节点分配且具有删除时间戳，则该 Pod 无法运行。
...
func (p *Pod) constructGroupPodSets() ([]kueue.PodSet, error) { // 同一个组里  ，pod 的规格会不同，类似  leader worker
...
func (p *Pod) Run(ctx context.Context, c client.Client, podSetsInfo []over_podset.PodSetInfo, recorder record.EventRecorder, msg string) error {
	log := ctrl.LoggerFrom(ctx)

	if !p.isGroup {
		if len(podSetsInfo) != 1 {
			return fmt.Errorf("%w: expecting 1 pod set got %d", over_podset.ErrInvalidPodsetInfo, len(podSetsInfo))
		}
	...
	return over_parallelize.Until(ctx, len(p.list.Items), func(i int) error {
		...
		if err := clientutil.Patch(ctx, c, pod, true, func() (bool, error) {
			...
			podSetIndex := slices.IndexFunc(podSetsInfo, func(info over_podset.PodSetInfo) bool {
				return string(info.Name) == roleHash
			})
			if podSetIndex == -1 {
				return false, fmt.Errorf("%w: podSetInfo with the name '%s' is not found", over_podset.ErrInvalidPodsetInfo, roleHash)
			}
