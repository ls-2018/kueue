...
func validateCreateForPrebuiltWorkload(jobOrPod GenericJob) field.ErrorList {
	var allErrs field.ErrorList
	allErrs = append(allErrs, ValidateLabelAsCRDName(jobOrPod.Object(), over_constants.PrebuiltWorkloadLabel)...)

	// this rule should be relaxed when its confirmed that running with a prebuilt wl is fully supported by each integration
	if _, hasPrebuilt := jobOrPod.Object().GetLabels()[over_constants.PrebuiltWorkloadLabel]; hasPrebuilt {
...
func validateCreateForMaxExecTime(jobOrPod GenericJob) field.ErrorList {
	if strVal, found := jobOrPod.Object().GetLabels()[over_constants.MaxExecTimeSecondsLabel]; found {
