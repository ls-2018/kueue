...
func expectedRunningPodSets(ctx context.Context, c client.Client, wl *kueue.Workload) []kueue.PodSet {
	...
	for i := range runningPodSets {
		...
		err := over_podset.Merge(&ps.Template.ObjectMeta, &ps.Template.Spec, *psi)
		if err != nil {
			return nil
		}
...
func getPodSetsInfoFromStatus(ctx context.Context, c client.Client, w *kueue.Workload) ([]over_podset.PodSetInfo, error) {
	if len(w.Status.Admission.PodSetAssignments) == 0 {
		return nil, nil
	}

	podSetsInfo := make([]over_podset.PodSetInfo, len(w.Status.Admission.PodSetAssignments))

	for i, psAssignment := range w.Status.Admission.PodSetAssignments {
		info, err := over_podset.FromAssignment(ctx, c, &psAssignment, w.Spec.PodSets[i].Count)
		if err != nil {
			return nil, err
		}
...
func (r *JobReconciler) getWorkloadForObject(ctx context.Context, jobObj client.Object) (*kueue.Workload, error) { // 获取与 pod\job 相关联的 workload
	...
	if err := r.client.List(ctx, &wlList, client.InNamespace(jobObj.GetNamespace()), client.MatchingFields{over_indexer.OwnerReferenceUID: string(jobObj.GetUID())}); client.IgnoreNotFound(err) != nil {
		return nil, err
	}
...
func GetPodSetsInfoFromWorkload(wl *kueue.Workload) []over_podset.PodSetInfo {
	if wl == nil {
		return nil
	}
...
func (r *JobReconciler) ensurePrebuiltWorkloadInSync(ctx context.Context, wl *kueue.Workload, jobOrPod GenericJob) (bool, error) {
	...
	if !equivalent || err != nil {
		...
		err := workload.UpdateStatus(ctx, r.client, wl,
			...
			over_constants.JobControllerName, r.clock)
		return false, err
	}
	return true, nil
}
...
func (r *JobReconciler) ReconcileGenericJob(ctx context.Context, req ctrl.Request, jobOrPod GenericJob) (result ctrl.Result, err error) {
	...
	if jobCond, ok := jobOrPod.(JobWithCustomWorkloadConditions); wl != nil && ok { // Pod
		if conditions, updated := jobCond.CustomWorkloadConditions(wl); updated { // ✅
			...
			return reconcile.Result{}, r.client.Status().Patch(ctx, wlPatch, client.Apply, client.FieldOwner(fmt.Sprintf("%s-%s-controller", over_constants.KueueName, strings.ToLower(jobOrPod.GVK().Kind))))
		}
	}
	...
	if message, success, finished := jobOrPod.Finished(); finished {
		...
		if wl != nil && !apimeta.IsStatusConditionTrue(wl.Status.Conditions, kueue.WorkloadFinished) {
			...
			err := workload.UpdateStatus(ctx, r.client, wl, kueue.WorkloadFinished, metav1.ConditionTrue, reason, message, over_constants.JobControllerName, r.clock)
			if err != nil && !apierrors.IsNotFound(err) {
				return ctrl.Result{}, err
			}
			r.record.Eventf(object, corev1.EventTypeNormal, ReasonFinishedWorkload, "Workload '%s' is declared finished", workload.Key(wl))
	...
	if r.waitForPodsReady { // 指示控制器是否应在对应的 job 所有 pod 就绪或成功时，向 workload 添加 PodsReady 条件。
		...
		if !workload.HasConditionWithTypeAndReason(wl, &condition) {
			...
			err := workload.UpdateStatus(ctx, r.client, wl, condition.Type, condition.Status, condition.Reason, condition.Message, over_constants.JobControllerName, r.clock)
			if err != nil {
	...
	if jobOrPod.IsSuspended() {
		...
		if workload.IsAdmitted(wl) {
			...
			if err != nil {
				...
				if over_podset.IsPermanent(err) {
					// 标记 workload 失败完成，因为无需重试
					errUpdateStatus := workload.UpdateStatus(ctx, r.client, wl, kueue.WorkloadFinished, metav1.ConditionTrue, FailedToStartFinishedReason, err.Error(), over_constants.JobControllerName, r.clock)
