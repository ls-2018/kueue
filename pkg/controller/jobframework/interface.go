...
type GenericJob interface {
	...
	RunWithPodSetsInfo(podSetsInfo []over_podset.PodSetInfo) error
	// RestorePodSetsInfo 会恢复作业的原始节点亲和性和 podSet 数量。
	// 返回是否有任何更改。
	RestorePodSetsInfo(podSetsInfo []over_podset.PodSetInfo) bool
...
type JobWithCustomStop interface {
	...
	// 返回此次调用是否停止了作业或错误。
	Stop(ctx context.Context, c client.Client, podSetsInfo []over_podset.PodSetInfo, stopReason StopReason, eventMsg string) (bool, error)
}
...
type ComposablePod interface {
	...
	Run(ctx context.Context, c client.Client, podSetsInfo []over_podset.PodSetInfo, r record.EventRecorder, msg string) error
	...
	Stop(ctx context.Context, c client.Client, podSetsInfo []over_podset.PodSetInfo, stopReason StopReason, eventMsg string) ([]client.Object, error)
...
func QueueNameForObject(object client.Object) kueue.LocalQueueName {
	if queueLabel := object.GetLabels()[over_constants.QueueLabel]; queueLabel != "" {
		return kueue.LocalQueueName(queueLabel)
	}
	// fallback to the annotation (deprecated)
	return kueue.LocalQueueName(object.GetAnnotations()[over_constants.QueueAnnotation])
}
...
func PrebuiltWorkloadFor(jobOrPod GenericJob) (string, bool) {
	name, found := jobOrPod.Object().GetLabels()[over_constants.PrebuiltWorkloadLabel]
	return name, found
}
...
func MaximumExecutionTimeSecondsForObject(object client.Object) *int32 {
	strVal, found := object.GetLabels()[over_constants.MaxExecTimeSecondsLabel]
	if !found {
		return nil
	}
...
func WorkloadPriorityClassName(object client.Object) string {
	if workloadPriorityClassLabel := object.GetLabels()[over_constants.WorkloadPriorityClassLabel]; workloadPriorityClassLabel != "" {
