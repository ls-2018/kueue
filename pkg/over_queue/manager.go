...
func WithAdmissionFairSharing(cfg *config.AdmissionFairSharing) Option {
	return func(o *options) {
		if over_features.Enabled(over_features.AdmissionFairSharing) {
...
func (m *Manager) DeleteClusterQueue(cq *kueue.ClusterQueue) {
	...
	over_metrics.ClearClusterQueueMetrics(cq.Name)
}
...
func (m *Manager) DeleteLocalQueue(q *kueue.LocalQueue) {
	...
	if over_features.Enabled(over_features.LocalQueueMetrics) {
		...
		over_metrics.ClearLocalQueueMetrics(over_metrics.LocalQueueReference{
			Name:      lqName,
			Namespace: namespace,
		})
	}
...
func (m *Manager) reportPendingWorkloads(cqName kueue.ClusterQueueReference, cq *ClusterQueue) {
	...
	over_metrics.ReportPendingWorkloads(cqName, active, inadmissible)
}
...
func (m *Manager) reportLQPendingWorkloads(lq *LocalQueue) {
	...
	over_metrics.ReportLocalQueuePendingWorkloads(over_metrics.LocalQueueReference{
		Name:      lqName,
		Namespace: namespace,
	}, active, inadmissible)
}
