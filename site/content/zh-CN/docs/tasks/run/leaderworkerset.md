...
date: 2025-02-17
weight: 6
description: 在启用了 Kueue 的环境里运行 LeaderWorkerSet
---
...
本指南适用于 [服务用户](/zh-CN/docs/tasks#serving-user) ，他们具有 Kueue 的基本理解。有关更多信息，请参见 [Kueue 概述](/zh-CN/docs/overview)。
...
你可以对 LeaderWorkerSet `.spec.replicas` 执行缩放操作。

缩放的单位是 LWS 组。通过更改 LWS 中的 `replicas` 数量，你可以创建或删除整个 Pod 组。缩放后，新创建的 Pod 组会通过调度门被挂起，直到相应的 Workload 被承认。
